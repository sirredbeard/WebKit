...
{
    // Local/in-process drags are web (or same-app) sources. They must not
    // receive filesystem content grants even if filenames were populated.
    if (drag.flags().contains(DragApplicationFlags::IsSource))
        return false;
    return true;
}
